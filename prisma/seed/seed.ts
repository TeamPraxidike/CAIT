...
const main = async () => {
		...
		data: {
			publication: {
				create: {
					...
					publisherId: 1,
					description: 'A circuit that walks you through the basics of machine learning.',
	...
	nodes.push(await addNode(circuit.id, materials[4].publicationId, -250, 250)); // perceptron 3
	nodes.push(await addNode(circuit.id, materials[5].publicationId, 0, 0)); // basics 4

	const edges = [
		{fromId: nodes[4].publicationId, toId: [nodes[3].publicationId, nodes[2].publicationId]},
