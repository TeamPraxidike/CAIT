<script lang="ts">
	...
	let allPublisherNames: { id: number, content: string }[] = users.map((x: any) => ({
		id: x.id,
		content: (x.firstNname + ' ' + x.lastName)
	...
	let allTypes: {
		id: number,
		content: string
	}[] = ['Presentation', 'Code', 'Video', 'Assignment', 'Dataset', 'Exam', 'Circuit'].map(x => ({
		id: 0,
		content: x
	})); //array with all the tags MOCK
	...
	{#if pageType !== "people"}
		<div class="flex gap-1 items-center">
			...
			{#if pageType === "materials"}
				<Filter label="Difficulty" bind:selected={selectedDiff} bind:all="{diffOptions}"
						bind:display="{diffOptions}"
			...
			{:else}
				<Filter label="Min Num Nodes" selected={[]} all="{[]}" display="{[]}" type="{true}"
						profilePic="{false}" bind:active="{diffActive}" on:clearSettings={clearAll}
						bind:num={numberNodes} />
			{/if}
			<div class="w-px h-4/5 bg-surface-600"></div>
			<Filter label="Sort By" profilePic="{false}" oneAllowed={true} bind:selectedOption={sortByText}
					bind:all={sortOptions} selected={[]} num="{0}" />
...
{#if pageType === "materials"}
	{#each materials as material}
		<PublicationCard extensions="{getExtensions(material)}" imgSrc={'data:image;base64,' + material.coverPicData}
						 publication={material.publication} liked={liked.includes(material.publication.id)}
