<script lang="ts">
    ...
    export let course:CourseWithMaintainersAndProfilePic;
    if (!course) {
        throw new Error("There was an error with exporting the course data. Please try again.");
    }
...
<div class="col-span-4 flex flex-col items-center gap-2 text-surface-800 rounded-b-lg pb-4 border border-surface-300 border-t-0 self-start
    ...
    <!--{/if}-->
    <img src={userPhotoUrl ? userPhotoUrl : defaultProfilePicturePath } alt="Profile picture of the course}"
