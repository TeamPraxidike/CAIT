<script lang="ts">
    ...
    let edited = ""
		...
		function getDateDifference(startDate:Date, endDate:Date):string {
		...
		const handleLike = () => {
			...
			dispatch("LikeAction", {message: liked ? "User added like":"User removed like", value:{action: likes, userId:user}})
		}
		const startEditing = () => {
			editing = true;
			newText = text;
...
<div bind:this={commentDiv} class="peer/comment {isReply ? 'col-start-2 md:col-start-2 md:col-span-7': 'col-start-1 md:col-start-1 md:col-span-8'} relative rounded-lg flex gap-2 p-1 " >
	...
	<div class="flex flex-col w-full">
		...
		<div class="flex gap-3 items-center">
			...
			<button class="[&>*]:pointer-events-none absolute right-0 hover:shadow-lg rounded-lg hover:bg-surface-200 dark:hover:bg-surface-800" use:popup={popupMenu}>
		...
		<div class="mb-2 w-full">
			{#if editing}
				<div>
					<textarea rows="5" class="border-b border-surface-200 border-opacity-50 dark:border-surface-100 dark:border-opacity-50 text-surface-800 text-opacity-90 dark:text-opacity-90 dark:bg-surface-800 dark:text-surface-50 w-full" bind:value={newText}/>
					<button class="float-right rounded-lg dark:bg-surface-800 hover:bg-surface-300 dark:hover:bg-surface-700 text-surface:700" on:click={saveChanges}>Save</button>
				</div>
				{:else }
				<p bind:this={content} class="text-surface-800 text-opacity-95 dark:text-opacity-95 dark:text-surface-50 mt-2 {lineClamp} text-md">{text}</p>
				<!--{#if truncated}-->
...
<div data-popup="{popupName}" >
	<div class="flex flex-col w-12 gap-2 bg-surface-200 dark:bg-surface-800 dark:text-surface-200 rounded-lg" >
		<button id="copyButton" on:click={copyToClipboard} class="rounded-lg hover:bg-surface-300 dark:hover:bg-surface-700">
			Copy
		</button>
<!--		only visible when user is the one who published, don't know how would work yey, just a setup-->
		{#if browsingUser === user}
			<button id="editButton" on:click={startEditing}  class="hover:bg-surface-300 dark:hover:bg-surface-700 rounded-lg">
				Edit
			</button>
			<button on:click={handleDelete} id="deleteButton" class="hover:bg-surface-300 dark:hover:bg-surface-700 rounded-lg">
				Delete
			</button>
		{/if}
	</div>
	...
	<div class="z-[9999] fixed inset-0 bg-surface-800 bg-opacity-50 flex justify-center items-center">
		<div class="bg-surface-50 dark:bg-surface-700 dark:text-surface-00 p-8 rounded-lg shadow-lg">
			...
			<div class="flex justify-center mt-4">
				<button class="mr-4 px-4 py-2 bg-error-500 text-surface-50 rounded-lg" on:click={confirmDelete}>Delete</button>
				<button class="px-4 py-2 bg-surface-500 text-surface-200 rounded-lg" on:click={cancelDelete}>Cancel</button>
			</div>
		</div>
	</div>
{/if}