    ...
    <Grid>
        ...
        <div class="hidden col-start-2 col-span-7 gap-2 lg:gap-4 items-center justify-center flex hidden md:flex">
            {#each navOptions as opt}
                <a
                  class=" group transition text-surface-800 dark:text-surface-50 h-full flex items-center justify-center md:px-2 xl:px-4"
                  href={opt.link}>
        ...
        <div class="hidden md:flex col-end-13 col-span-2 md:gap-2 xl:gap-4 items-center justify-self-end">
            {#if loggedIn}
                <button
                  class="hidden md:block btn rounded-lg md:py-1 lg:py-1.5 md:px-2 lg:px-3  bg-primary-600 text-surface-50 hover:opacity-60 transition duration-400">
                    Publish
                </button>
            {:else}
                <button
        ...
        <button class="col-end-5 self-center justify-self-end w-12 h-8 md:hidden" on:click={toggleDropDown}>
            <Icon icon="solar:hamburger-menu-outline" className="text-surface-600 dark:text-surface-50" width="48"
                  height="32" />
        </button>
        {#if dropDown}
