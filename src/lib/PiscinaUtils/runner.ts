...
import type {PublicationMeta, ResultMeta} from "$lib/PiscinaUtils/worker2";

const __filenameCurrent = fileURLToPath(import.meta.url);
const __dirnameCurrent = path.dirname(__filenameCurrent);

const pool = workerpool.pool(path.resolve(__dirnameCurrent, './worker2'), {
    maxWorkers: 4
});
...
async function compareNodesInBackground(pubANodes: number[], pubBNodes: number[]): Promise<number> {
    return pool.exec('compareNodes', [pubANodes, pubBNodes]);
}
...
async function compareMetaInBackground(pubAMeta: PublicationMeta, pubBMeta: PublicationMeta): Promise<ResultMeta> {
    return pool.exec('compareMeta', [pubAMeta, pubBMeta]);
}
...
async function compareFilesInBackground(pubAFiles: PrismaFile[], pubBFiles: PrismaFile[]): Promise<number> {
    return pool.exec('compareFiles', [pubAFiles, pubBFiles]);
}
...
export async function enqueueMaterialComparison(publicationId: number): Promise<void> {
    try {
        ...
        const comparisons: {
            fromPubId: number,
            toPubId: number,
            similarityFile: Promise<number>,
            similarityMeta: Promise<ResultMeta>
        }[] = [];
        ...
        for (let i = 0; i < circuits.length; i++) {
            if (circuits[i].publicationId !== publicationId) {
                ...
                comparisons.push({
                    ...
                    similarityNode: compareNodesInBackground(
                        currentCircuit!.nodes.map(n => n.publicationId),
