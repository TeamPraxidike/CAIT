 ...
 * @param pubBNodes
 */
export async function compareNodes(pubANodes: number[], pubBNodes: number[]){
    return jaccardComparison(pubANodes, pubBNodes)
}

export async function compareFiles(pubAFiles: PrismaFile[], pubBFiles: PrismaFile[]) {
    return compare(pubAFiles, pubBFiles);
}

export async function compareMeta(pubA: PublicationMeta, pubB: PublicationMeta): Promise<ResultMeta> {
